...
ModuleSceneKen::ModuleSceneKen(bool start_enabled) : Module(start_enabled)
{
	...
	//shipAnim.frames.push_back({ 8, 28, 521, 181 });
	//shipAnim.frames.push_back({ 8, 26, 521, 181 });
	//shipAnim.speed = 0.08f;
}
...
bool ModuleSceneKen::Start()
{
	...
	App->audio->PlayMusic("ken.ogg", DEFAULT_MUSIC_FADE_TIME); //my code
	return true;
}
...
update_status ModuleSceneKen::Update()
{
	...
	//App->renderer->Blit(graphics, 0, 0, &ship, 2.0f); // ship
	// Draw everything --------------------------------------
	...
	App->renderer->Blit(graphics, 0, posY, &ship, 2.0f); // ship
